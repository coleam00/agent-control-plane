...
// Run the migration file statement-by-statement (Neon HTTP runs one at a time).
export async function initSchema(): Promise<void> {
  const file = new URL("../migrations/001_init.sql", import.meta.url);
  const text = await Bun.file(file).text();
  // Strip whole-line SQL comments first, THEN split on ';'. (Splitting first
  // would leave the file's leading comment block attached to the first
...
export async function createLoop(input: {
  ...
  const rows = (await sql`
    insert into loops (goal, max_iterations, model, workspace, status)
    values (${input.goal}, ${input.maxIterations}, ${input.model}, ${input.workspace}, 'pending')
    returning *
  `) as Loop[];
  return rows[0]!;
}
...
export async function startRun(input: {
  ...
  const rows = (await sql`
    insert into runs (loop_id, iteration, task, model, status)
    values (${input.loopId}, ${input.iteration}, ${input.task}, ${input.model}, 'running')
    returning *
  `) as Run[];
  return rows[0]!;
}
...
export async function listRunsForLoop(loopId: string): Promise<Run[]> {
  return (await sql`
    select * from runs where loop_id = ${loopId} order by iteration asc
  `) as Run[];
}
